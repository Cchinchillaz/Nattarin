...
function Experiences() {
    ...
    return (
        <section className='c-space my-10 w-full flex flex-col justify-center' id='experiences'>
                ...
                <div className="grid grid-cols-3 grid-rows-3 gap-0">
                    {experiences.map((exp) => (
                        <div className="col-span-1 row-span-1 relative w-full pb-[100%]" key={exp.id}>
                            ...
                            <div className="absolute inset-0 bg-black/50 rounded-lg opacity-0 hover:opacity-60 cursor-pointer" onClick={() => handleEventClick(exp)} onTouchStart={() => handleEventClick(exp)}></div>
                        </div>
                    ))}
                </div>
            </div>
